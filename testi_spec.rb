RSpec.describe 'luokka Varipiste' do
  ...
  it "metodi etaisyys_origosta toimii" do
    ...
    expect((p1.etaisyys_origosta - 5).abs).to be <= 0.000001

    p1 = Varipiste.new 1, 1, :vihrea
    expect((p1.etaisyys_origosta - Math.sqrt(2)).abs).to be <= 0.000001
  end
...
RSpec.describe 'luokka Piste3d' do
  ...
  it "metodi etaisyys_origosta toimii" do
    ...
    expect((p1.etaisyys_origosta - 6).abs).to be <= 0.000001

    p1 = Piste3d.new 1, 1, 1
    expect((p1.etaisyys_origosta - Math.sqrt(3)).abs).to be <= 0.000001
  end
